...
.skip_name ;Notes are as they appear in the tutorial
	ld a, [wTrainerGroupBank] ;ld a, BANK(Trainers)
	call GetFarByte
	inc hl
	cp "@"
	jr nz, .skip_name
...
; TRAINERTYPE_NICKNAME has uneven length, so always use the first mon
	bit TRAINERTYPE_NICKNAME_F, c ;bit TRAINERTYPE_NICKNAME_F, b
	jr nz, .got_mon
; c = mon length
; All trainers use 2 bytes for level and species
	ld a, 3 ;ld c, 2
; TRAINERTYPE_ITEM uses 1 more byte
	bit TRAINERTYPE_ITEM_F, c ;bit TRAINERTYPE_ITEM_F, b
	jr z, .no_item
	inc a
.no_item
